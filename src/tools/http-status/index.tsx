      ...
      <div className="space-y-4">
        <div className="flex gap-4 flex-wrap">
          <input
            ...
            className="flex-1 min-w-48 px-3 py-2 bg-bg-secondary border border-border-primary rounded-lg text-text-primary placeholder-text-tertiary focus:outline-none focus:ring-2 focus:ring-accent-primary text-sm"
          />

          <select
            value={selectedCategory || ''}
            onChange={(e) => setSelectedCategory(e.target.value || null)}
            className="px-3 py-2 bg-bg-secondary border border-border-primary rounded-lg text-text-primary text-sm focus:outline-none focus:ring-2 focus:ring-accent-primary"
          >
        ...
        <div className="grid gap-2">
          {filteredStatus.map((status) => (
            <div
              key={status.code}
              className="p-3 bg-bg-secondary border border-border-primary rounded-lg flex items-start gap-3"
            >
              ...
              <span className="text-xs text-text-tertiary">{CATEGORY_LABELS[status.category]}</span>
            </div>
          ))}
        </div>

        {filteredStatus.length === 0 && (
          <div className="text-center text-text-tertiary py-8">未找到匹配的状态码</div>
        )}
      </div>
    </ToolLayout>
  )
